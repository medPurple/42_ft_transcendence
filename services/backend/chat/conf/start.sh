...
sleep 5
data=$(curl -H "X-Vault-Token: $(cat /run/secrets/vault_token_chat)" http://vault:8200/v1/kv/nginx | jq -r '.data'| sed 's/\\n/\\\\n/g')
# echo $data
...
uvicorn chat.asgi:application --host 0.0.0.0 --port 4430 --ssl-keyfile=/tmp/server.key --ssl-certfile=/tmp/server.crt --reload