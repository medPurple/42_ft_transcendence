...
websocket_urlpatterns = [
    ...
    re_path("ws/pong/chat/(?P<user_id>\w+)/(?P<user_name>\w+)/$", consumers.PongChatConsumer.as_asgi()),
    re_path("ws/pong/local/(?P<user_id>\w+)/(?P<user1>\w+)/(?P<user2>\w+)/$", consumers.PongLocalConsumer.as_asgi()),
